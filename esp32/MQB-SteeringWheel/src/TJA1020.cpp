...
void Lin_TJA1020::setMode(TJA1020_Mode mode)
{
    ...
    pinMode(pin_tx, OUTPUT);   //  TX  Signal to LIN Tranceiver
...
#ifdef ESP32
        pinMode(pin_tx, INPUT_PULLDOWN); // ensure Low level while in sleep mode (since TJA1020 has internally a fixed pulldown)
        pinMode(_nslp_pin, INPUT_PULLDOWN); // ensure Low level while in sleep mode
#else
        pinMode(pin_tx, INPUT); // ensure Low level while in sleep mode (since TJA1020 has internally a fixed pulldown)
